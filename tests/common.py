...
from config_resolver import NoVersionError, from_string, get_config
...
class CommonTests:
    ...
    def test_env_name(self):
        with environment(
            HELLO_WORLD_FILENAME=self.TEST_FILENAME,
            XDG_CONFIG_HOME="",
            XDG_CONFIG_DIRS="",
        ):
            result = get_config("world", "hello", handler=self.HANDLER_CLASS)
        expected = [
            "/etc/hello/world/%s" % self.TEST_FILENAME,
        ...
        self.assertEqual(result.meta.active_path, expected)

    def test_env_name_override(self):
        with environment(HELLO_WORLD_FILENAME=self.TEST_FILENAME):
            get_config("world", "hello", handler=self.HANDLER_CLASS)
        msg = (
            "filename was overridden with '%s' by the environment "
            "variable HELLO_WORLD_FILENAME" % self.TEST_FILENAME
        )
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.INFO, msg
        )
    ...
    def test_env_path_add(self):
        path = "+{0}:{0}/a:{0}/b".format(self.DATA_PATH)
        with environment(
            HELLO_WORLD_PATH=path, XDG_CONFIG_HOME="", XDG_CONFIG_DIRS=""
        ):
            result = get_config("world", "hello", handler=self.HANDLER_CLASS)
        expected = [
    ...
    def test_env_path_add_log(self):
        ...
        with environment(HELLO_WORLD_PATH=path):
            get_config("world", "hello", handler=self.HANDLER_CLASS)
        msg = (
            "extended with '%s' by the " "environment variable HELLO_WORLD_PATH"
        ) % path
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.INFO, msg
        )
    ...
    def test_unsecured_logmessage(self):
        ...
        catcher.assert_contains(
            "config_resolver.hello.world", logging.WARNING, expected_message
        )
    ...
    def test_unsecured_file(self):
        ...
        self.assertNotIn(
            join(self.DATA_PATH, self.TEST_FILENAME), result.meta.loaded_files
        )
    ...
    def test_secured_nonexisting_file(self):
        ...
        self.assertNotIn(
            join(self.DATA_PATH, "nonexisting.ini"), result.meta.loaded_files
        )
    ...
    def test_mismatching_major(self):
        ...
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.ERROR, "2.1"
        )
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.ERROR, "1.1"
        )
    ...
    def test_mismatching_minor(self):
        ...
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.WARNING, "2.2"
        )
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.WARNING, "2.1"
        )
    ...
    def test_mixed_version_load(self):
        ...
        get_config(
            ...
            lookup_options={
                ...
                "search_path": "{0}/versioned:{0}/versioned2".format(
                    self.DATA_PATH
                ),
            },
            handler=self.HANDLER_CLASS,
        ...
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.ERROR, "1.0"
        )
        self.catcher.assert_contains(
            "config_resolver.hello.world", logging.ERROR, "2.0"
        )

    def test_xdg_config_dirs(self):
        with environment(
            XDG_CONFIG_DIRS="/xdgpath1:/xdgpath2", XDG_CONFIG_HOME=""
        ):
            result = get_config("bar", "foo", handler=self.HANDLER_CLASS)
            self.assertEqual(
                [
    ...
    def test_xdg_config_home(self):
        with environment(
            XDG_CONFIG_HOME="/path/to/config/home", XDG_CONFIG_DIRS=""
        ):
            result = get_config("bar", "foo", handler=self.HANDLER_CLASS)
            self.assertEqual(
                [
    ...
    def test_both_xdg_variables(self):
        with environment(
            XDG_CONFIG_DIRS="/xdgpath1:/xdgpath2",
